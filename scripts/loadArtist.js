...
let genreArtists = [
	...
	{
		genreName: 'Reggae',
		genreArtists: ["Bob Marley", "Burning Spear", "UB40", "Bunny Wailer", "U-Roy", "Steel Pulse", "Max Romeo", "Alborosie", "Clinton Fearon", "Manu Chao", "Peter Tosh", "Jah Cure", "Maxi Priest", "Sean Paul", "Flox"],
	}
]

async function loadArtists(selectedGenre) {
    mainArea.innerHTML = '';
    const genre = genreArtists.find(genre => genre.genreName.toLowerCase() === selectedGenre.toLowerCase());
    if (!genre) {
        ...
        const artistPromises = genre.genreArtists.map(async (genreArtist) => {
            ...
            if (data.data && Array.isArray(data.data)) {
                data.data.forEach((songData) => {
                    ...
                    if (isArtistInGenre(artistName.toLowerCase(), genre.genreArtists) &&
...
function isArtistInGenre(artistName, genreArtists) {
    return genreArtists.some(genreArtist => genreArtist.toLowerCase() === artistName);
}
